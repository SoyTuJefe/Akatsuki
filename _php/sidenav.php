<div id="main-sidebar">
            ...
            <div class="cbox cbox-genres">
                <ul class="ulclear color-list sb-genre-list sb-genre-less">
                    ...
                    <li class="nav-item"> <a class="nav-link" href="../genre/horror" title="Horror">Horror</a> </li>
                       <li class="nav-item"> <a class="nav-link" href="../genre/isekai" title="Magic">Isekai</a></li>
                    ...
                    <li class="nav-item"> <a class="nav-link" href="../genre/reincarnation" title="Romance">Reincarnation</a> </li>
    ...
    <section class="block_area block_area_sidebar block_area-realtime">
        <div class="block_area-header">
            <div class="float-left bah-heading mr-2">
                <h2 class="cat-heading">Top 10 Ongoing</h2>
            </div>
            ...
            <div class="cbox cbox-list cbox-realtime">
                <div class="cbox-content">
                    <div class="tab-content">
                        ...
                        <div class="film-number"> <span><?= ($key < 9) ? '0' . ($key + 1) : ($key + 1) ?></span></div>
                                    <div class="film-poster">
                                        <img data-src="<?=$trending['animeImg']?>"
                                            class="film-poster-img lazyload tooltipEl" alt="<?=$trending['animeTitle']?>"
    ...
    // Use the current week number as the random seed for shuffling
    srand($currentWeek);
    ...
    foreach ($json as $key => $trending) {
        ...
        <li class="<?php if ($key < 3) echo "item-top" ?>">
            <div class="film-number"> <span><?= ($key < 9) ? '0' . ($key + 1) : ($key + 1) ?></span></div>
            <div class="film-poster">
                <img data-src="<?= $trending['animeImg'] ?>" class="film-poster-img lazyload tooltipEl" alt="<?= $trending['animeTitle'] ?>" src="<?= $trending['animeImg'] ?>" title="<?= $trending['animeTitle'] ?>">
            </div>
            <div class="film-detail">
                <h3 class="film-name">
                    <a href="<?= $websiteUrl ?>/anime/<?= $trending['animeId'] ?>" title="<?= $trending['animeTitle'] ?>" data-jname="<?= $trending['animeTitle'] ?>"><?= $trending['animeTitle'] ?></a>
                </h3>
                <div class="fd-infor">
                    <span class="fdi-item"><?= $trending['latestEp'] ?></span>
                </div>
    ...
    <?php } ?>
                        ...
                        <div id="top-viewed-month" class="anif-block-ul anif-block-chart tab-pane">
                            <ul class="ulclear">
                            <?php
                                $json = file_get_contents("$api/top-airing?page=2");
                                $json = json_decode($json, true);
                                foreach($json as $key => $trending) { ?>
                                <li class="<?php if($key < 3) echo "item-top"?>">
                        <div class="film-number"> <span><?= ($key < 9) ? '0' . ($key + 1) : ($key + 1) ?></span></div>
