...
foreach($json as $key => $az) {
    // Extract the image src using regex
    $imageSrc = '';
    preg_match('/<img\s+src="([^"]+)"/', $az['liTitle'], $matches);
    if (isset($matches[1])) {
        $imageSrc = $matches[1];
    }
?>
...
<div class="flw-item">
    ...
    <div class="film-poster">
        <div class="tick ltr">
            <!-- Rest of your code here -->
		 <div class="tick-item-<?php $str = $az['animeTitle'];
                                                  $last_word_start = strrpos ( $str , " ") + 1;
                                                  $last_word_end = strlen($str) - 1;
                                                  $last_word = substr($str, $last_word_start, $last_word_end);
                                                  if ($last_word == "(Dub)"){echo "dub";} else {echo "sub";}
                                                ?>   tick-eps amp-algn"><?php $str = $az['animeTitle'];
                                                $last_word_start = strrpos ( $str , " ") + 1;
                                                $last_word_end = strlen($str) - 1;
                                                $last_word = substr($str, $last_word_start, $last_word_end);
                                                if ($last_word == "(Dub)"){echo "DUB";} else {echo "SUB";}
        ...
        <!-- Use the extracted image src in the data-src attribute -->
        <img class="film-poster-img lazyload"
            data-src="<?= $imageSrc ?>"
            src="<?= $websiteUrl ?>/files/images/no_poster.jpg"
            alt="<?= $az['animeTitle'] ?>">
        <a class="film-poster-ahref"
            href="/anime/<?= $az['animeId'] ?>"
            title="<?= $az['animeTitle'] ?>"
            data-jname="<?= $az['animeTitle'] ?>"><i class="fas fa-play"></i></a>
    </div>
...
?>
    ...
    <nav aria-label="Page navigation">
        <ul class="pagination pagination-lg justify-content-center">
            <?php 
            $currentPage = isset($_GET['page']) ? intval($_GET['page']) : 1; // Using $_GET to get the current page number.
            $totalPages = 300; // Replace this with the total number of pages in your API response.
            ...
            // "First" link
            if ($currentPage > 1) {
                echo '<li class="page-item"><a title="First" class="page-link" href="/az-list/'.$url.'?page=1">&laquo;</a></li>';
            }

            // "Previous" link
            if ($currentPage > 1) {
                echo '<li class="page-item"><a title="Previous" class="page-link" href="/az-list/'.$url.'?page=' . ($currentPage - 1) . '">&lsaquo;</a></li>';
            }
            ...
            for ($i = $startPage; $i <= $endPage; $i++) {
                if ($i === $currentPage) {
                    ...
                    echo '<li class="page-item"><a title="Page ' . $i . '" class="page-link" href="/az-list/'.$url.'?page=' . $i . '">' . $i . '</a></li>';
                }
            }

            // "Next" link
            if ($currentPage < $totalPages) {
                echo '<li class="page-item"><a title="Next" class="page-link" href="/az-list/'.$url.'?page=' . ($currentPage + 1) . '">&rsaquo;</a></li>';
            }

            // "Last" link
            // if ($currentPage < $totalPages) {
            //     echo '<li class="page-item"><a title="Last" class="page-link" href="/az-list/'.$url.'?page=' . $totalPages . '">&raquo;</a></li>';
            // }
            ?>
        </ul>
    </nav>
