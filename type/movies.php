...
<head>
    ...
    <title>Anime Movies on <?=$websiteTitle?></title>
    
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <meta name="title" content="Anime Movies on <?=$websiteTitle?>">
    ...
    <meta property="og:title" content="Anime Movies on <?=$websiteTitle?>">
    <meta property="og:description" content="Anime Movies on <?=$websiteTitle?> in HD with No Ads. Watch anime online">
...
<body data-page="page_anime">
                    ...
                    <section class="block_area block_area_category">
                        <div class="block_area-header">
                            <div class="float-left bah-heading mr-4">
                                <h2 class="cat-heading">Anime Movies</h2>
                            </div>
                            <div class="clearfix"></div>
                        </div>
                        <div class="tab-content">
                            <div class="block_area-content block_area-list film_list film_list-grid film_list-wfeature">
                                ...
                                <?php 
                                $json = file_get_contents("$api/anime-movies?page=$page");
                                $json = json_decode($json, true);
                                foreach($json as $key => $movie) { ?>
                                    <div class="flw-item">
                                        <div class="film-poster">
                                        <div class="tick ltr">
                                                <div class="tick-item-<?php $str = $movie['animeTitle'];
                                                  $last_word_start = strrpos ( $str , " ") + 1;
                                                  $last_word_end = strlen($str) - 1;
                                                  $last_word = substr($str, $last_word_start, $last_word_end);
                                                  if ($last_word == "(Dub)"){echo "dub";} else {echo "sub";}
                                                ?>  tick-eps amp-algn"><?php $str = $movie['animeTitle'];
                                                $last_word_start = strrpos ( $str , " ") + 1;
                                                $last_word_end = strlen($str) - 1;
                                                $last_word = substr($str, $last_word_start, $last_word_end);
                                                if ($last_word == "(Dub)"){echo "DUB";} else {echo "SUB";}
                                            ...
                                            <a class="film-poster-ahref"
                                                ...
                                                data-jname="<?=$movie['animeTitle']?>"><i class="fas fa-play"></i></a>
                                        </div>
                                        <div class="film-detail">
                                            <h3 class="film-name">
                                                <a
                                                    href="/anime/<?=$movie['animeId']?>"
                                                    title="<?=$movie['animeTitle']?>"
                                                    data-jname="<?=$movie['animeTitle']?>"><?=$movie['animeTitle']?></a>
                                            </h3>
                                            <div class="description"></div>
                                            <div class="fd-infor">
                                                <span class="fdi-item"><?=$movie['status']?></span>
                                            </div>
                                        </div>
                                        <div class="clearfix"></div>
                                    </div>
   ...
   <nav aria-label="Page navigation">
    <ul class="pagination pagination-lg justify-content-center">
        <?php 
        $currentPage = $page; // Assuming the current page number is stored in the $page variable.
        $totalPages = 106; // Replace this with the total number of pages in your API response.

        // Generate the pagination links.
        // Assuming you want to display a fixed number of pages (e.g., 5) in the pagination bar.
        $numLinks = 4;
        ...
        // "First" link
        if ($currentPage > 1) {
            echo '<li class="page-item"><a title="First" class="page-link" href="/type/movies?page=1">&laquo;</a></li>';
        }

        // "Previous" link
        if ($currentPage > 1) {
            echo '<li class="page-item"><a title="Previous" class="page-link" href="/type/movies?page=' . ($currentPage - 1) . '">&lsaquo;</a></li>';
        }
        ...
        for ($i = $startPage; $i <= $endPage; $i++) {
            if ($i === $currentPage) {
                ...
                echo '<li class="page-item"><a title="Page ' . $i . '" class="page-link" href="/type/movies?page=' . $i . '">' . $i . '</a></li>';
            }
        }

        // "Next" link
        if ($currentPage < $totalPages) {
            echo '<li class="page-item"><a title="Next" class="page-link" href="/type/movies?page=' . ($currentPage + 1) . '">&rsaquo;</a></li>';
        }

        // "Last" link
        if ($currentPage < $totalPages) {
            echo '<li class="page-item"><a title="Last" class="page-link" href="/type/movies?page=' . $totalPages . '">&raquo;</a></li>';
        }
        ?>
    </ul>
</nav>
